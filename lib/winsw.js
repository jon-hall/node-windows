module.exports = {
  ...
  generateXml: function(config){
    ...
    if(config.user) {
        xml += '<serviceaccount>';
        xml += '<domain>'+config.user.domain+'</domain>';
        xml += '<user>'+config.user.account+'</user>';
        xml += '<password>'+config.user.password+'</password>';
        xml += '<allowservicelogon>true</allowservicelogon>';
        xml += '</serviceaccount>';
    }
